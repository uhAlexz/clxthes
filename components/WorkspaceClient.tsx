...
import { fetchGroupCatalogs } from "@/lib/catalogCache";
...
export default function WorkspaceClient() {
  ...
  const storeHydrated = useStore((s) => s._hasHydrated);
  ...
  const isReady = mounted && storeHydrated;
  ...
  const { data, error, isLoading } = useSWR(
    hasGroups ? `catalog:${activeGroups.join(",")}` : null,
    () => fetchGroupCatalogs(activeGroups),
    {
  ...
  const catalogItems: ItemProps[] = data || [];
          ...
          <div className="flex flex-1 flex-col md:flex-row">
            ...
            <div className="flex-1 bg-background p-6 md:p-8">
              {isLoading && !data && !showSavedOnly ? (
                ...
                <motion.div layout className="grid grid-cols-2 gap-4 md:grid-cols-4 xl:grid-cols-5">
                  <AnimatePresence>
